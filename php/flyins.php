<?php
  ...
  class summerP {
    function summerP($nameP, $descriptionP, $linkP, $selectivityP, $locationP, $financialAidP, $descriptionDateP, $metricDateP, $tagsP, $gradesP, $filterDate) {
      $this->name = $nameP;
  ...
  $sql = "select *, (DAYOFYEAR(FilterDate) - DAYOFYEAR(CURDATE())) as days from `flyins` where MOD(DAYOFYEAR(FilterDate) - DAYOFYEAR(CURDATE()) + 365, 365) order by CASE WHEN days < 0 THEN 1 ELSE 0 END, days";
  $result = $conn->query($sql);

  $oppCount = 0;
  function printData($link, $location, $description, $descriptionDate, $grades, $tags, $financialAid, $metricDate){
		global $oppCount;
	  	$oppCount++;
        ...
        echo "</div>";
	  	if ($oppCount % 2 == 0) {
	  	  ...
	  	  echo "<div class=\"container\">";
	  	    echo "<h2 class=\"oppName\">". $location ."</h2>";
            $location = preg_replace('/\s+/', '', $location);
            $location = preg_replace('/\(|\)/','',$location);
            $location = str_replace("'", '', $location);
            $location = preg_replace('/[^a-z\d]+/i', '', $location);
            echo "<i class=\"fa fa-plus-circle addDesktop\" id=\"$location\"></i><br><br>";
            ...
            echo "<h2><a class=\"applyLink\" id=\"$location\" href=\"". $link ."\" target=\"_blank\">Apply Now<i class=\"fa fa-chevron-circle-right\"></i></h2></a>";
	      echo "</div>";
	    echo "</div>";
	    echo "</div>";
  ...
  if ($result->num_rows > 0) {
    ...
    $i = 0;
      ...
      while($row = $result->fetch_assoc()) {
        ...
        $qwerty = new summerP($name, $description, $link, $selectivity, $location, $financialAid, $descriptionDate, $metricDate, $tags, $grades, $filterDate);
        $qname = $qwerty->name;
        $arr[$qname] = $qwerty;
              ...
              if (metricFilter($grades,$_POST['grade'])){
                ...
                foreach ($arraryNew as $value) {
                  ...
                  if (metricFilter($tags,$value)){
                    printData($link, $location, $description, $descriptionDate, $grades, $tags, $financialAid, $filterDate);
					break;
                  }
                  else{
                    echo "";
