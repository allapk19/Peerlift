<?php
  ...
  class summerP {
    function summerP($nameP, $descriptionP, $linkP, $selectivityP, $locationP, $conferenceP, $descriptionDateP, $metricDateP, $tagsP, $gradesP, $networkP, $descriptionAmountP, $metricAmountP, $filterDate) {
      $this->name = $nameP;
	  ...
	  $this->descriptionAmount = $descriptionAmountP;
	  $this->metricAmount = $metricAmountP;
    ...
    public function __toString() {
	  ...
	  $network = 0;
	  $conference = 0;
	  $metricAmount = $this->metricAmount;
		
	  if ($this->metricAmount == 0) {
		  $metricAmount = 9999999999;
	  }
      ...
      return "{$difference}\t{$metricAmount}\t{$network}\t{$selectivity}\t{$conference}\tname{$this->name}endName\t";
    }
  }
  // create an object
  ...
  $sql = "select *, (DAYOFYEAR(FilterDate) - DAYOFYEAR(CURDATE())) as days from `scholarships` where MOD(DAYOFYEAR(FilterDate) - DAYOFYEAR(CURDATE()) + 365, 365) order by CASE WHEN days < 0 THEN 1 ELSE 0 END, days";
  $result = $conn->query($sql);


  $oppCount = 0;
    function printData($link, $name, $description, $descriptionDate, $grades, $tags, $descriptionAmount, $metricDate){
		global $oppCount;
	  	$oppCount++;	  
	  	...
	  	if ($oppCount % 2 == 0) {
            ...
            echo "<i class=\"fa fa-plus-circle addDesktop\" id=\"$name\"></i><br><br>";
	  	    echo "<div class=\"flex-container\">";
	  	      ...
	  	      echo "<div class=\"selectivity\"><h3>". $descriptionAmount ."</h3></div>";
  ...
  if ($result->num_rows > 0) {
    ...
    $i = 0;
      ...
      while($row = $result->fetch_assoc()) {
		...
		$network = $row["Network"];
		$descriptionAmount = $row["DescriptionAmount"];
		$metricAmount = $row["MetricAmount"];
		$filterDate = $row["FilterDate"];
        $qwerty = new summerP($name, $description, $link, $selectivity, $location, $conference, $descriptionDate, $metricDate, $tags, $grades, $network, $descriptionAmount, $metricAmount, $filterDate);
        $qname = $qwerty->name;
        $arr[$qname] = $qwerty;
      ...
      for($a = 0; $a<count($summerProgramOpArrayString); $a++){
		...
		$descriptionAmount = $arr[$a3]->descriptionAmount;
		$metricAmount = $arr[$a3]->metricAmount;
              ...
              if (metricFilter($grades,$_POST['grade'])){
                ...
                foreach ($arraryNew as $value) {
                  ...
                  if (metricFilter($tags,$value)){
                    printData($link, $name, $description, $descriptionDate, $grades, $tags, $descriptionAmount, $filterDate);
					break;
                  }
                  else{
                    echo "";
