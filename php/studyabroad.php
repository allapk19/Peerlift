<?php
  ...
  class StudyA {
    function studyA($nameP, $descriptionP, $linkP, $locationP, $financialAidP, $selectivityP, $descriptionDateP, $metricDateP, $tagsP, $gradesP, $filterDate) {
      $this->name = $nameP;
  ...
  $sql = "select *, (DAYOFYEAR(FilterDate) - DAYOFYEAR(CURDATE())) as days from `studyabroad` where MOD(DAYOFYEAR(FilterDate) - DAYOFYEAR(CURDATE()) + 365, 365) order by CASE WHEN days < 0 THEN 1 ELSE 0 END, days";
  $result = $conn->query($sql);

  $oppCount = 0;
  function printData($link, $name, $description, $descriptionDate, $grades, $tags, $financialAid, $metricDate){
		global $oppCount;
	  	$oppCount++;
  ...
  if ($result->num_rows > 0) {
    ...
    $i = 0;
      ...
      while($row = $result->fetch_assoc()) {
        ...
        $qwerty = new studyA($name, $description, $link, $location, $financialAid, $selectivity, $descriptionDate, $metricDate, $tags, $grades, $filterDate);
        $qname = $qwerty->name;
        $arr[$qname] = $qwerty;
              ...
              if (metricFilter($grades,$_POST['grade'])){
                ...
                foreach ($arraryNew as $value) {
                  ...
                  if (metricFilter($tags,$value)){
                    printData($link, $name, $description, $descriptionDate, $grades, $tags, $financialAid, $filterDate);
					break;
                  }
                  else{
                    echo "";
